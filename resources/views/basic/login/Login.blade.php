...
<head>
    ...
    <link rel="icon" type="image/x-icon" href="{{ asset('assets/img/custom/' . $satu->icon) }}" />
    ...
    <style>
        body {
            background-image: url('{{ asset('assets/img/custom/sph.jpg') }}'); /* Set background image */
                ...
                <div class="card" style="background-color: rgba(255, 255, 255, 0.8);"> <!-- Added transparency to the card -->
                    <div class="card-body">
                        ...
                        <div class="app-brand demo text-center">
                            @if (!empty($satu->logos))
                                <img src="{{ asset('assets/img/custom/' . $satu->logos) }}" alt="Login Icon"
                                    class="img-fluid mb-3 logo-login" style="max-width: 350px;"> 
                            @endif
                        </div>
                        <!-- /Logo -->

                        <form id="formAuthentication" class="mb-3" action="{{ url('aksi_login') }}" method="POST">
                            @csrf
                            <div class="mb-3">
